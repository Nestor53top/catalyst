...
namespace systems {
    ...
    void voxels::update( )
    {
        ...
        for ( const auto& proj : projectiles )
        {
            ...
            if ( !state || !rgba || state < 0x10000 || rgba < 0x10000 || frame < 0 || frame > 1024 )
                continue;
    ...
    {
        if ( this->m_active_voxels.empty( ) ) return false;
        const float half_size = 11.0f;
        for ( const auto& v : this->m_active_voxels )
        {
            ...
            if ( systems::g_bvh.intersect_segment_aabb( start, end, bmin, bmax ) )
                return true;
        }
        return false;
    }
