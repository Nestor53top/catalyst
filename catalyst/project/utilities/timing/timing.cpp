...
namespace timing {
    ...
    limiter::limiter( const std::uint32_t target_tps )
        : m_start_time{ }, m_target_frame_duration( 0 ), m_qpc_frequency( 0 )
    {
        if ( target_tps == 0 )
        {
    ...
    thread_local std::chrono::steady_clock::time_point stopwatch::m_start{};
    thread_local std::atomic<float> stopwatch::m_milliseconds{};
